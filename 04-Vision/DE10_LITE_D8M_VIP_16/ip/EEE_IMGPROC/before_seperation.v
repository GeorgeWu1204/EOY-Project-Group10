...
assign color_high  =  
					  ...
					  (white_final_detected) ? {24'hf0f0f0} : 
					  ...
					  (blue_final_detected) ? (24'h6151f4):
						//for testing
					//	(whiteToBlack) ? (24'hff005d):
					//	(blackToWhite) ? (24'h51e5f4):
...
assign mid_deviation_suspect_b_to_w =  ((estimatated_region_end_b + estimatated_region_start_b) > (max_start_edge_x_position_b + max_end_edge_x_position_b)) ? 
							((estimatated_region_end_b + estimatated_region_start_b) - (suspect_miss_b_to_w_count + 2 * suspect_miss_b_to_w_start ))
							: ((suspect_miss_b_to_w_count + 2 * suspect_miss_b_to_w_start ) - ( estimatated_region_end_b + estimatated_region_start_b ));

assign mid_deviation_suspect_w_to_b =  ((estimatated_region_end_b + estimatated_region_start_b) > (suspect_miss_w_to_b_count + 2 * suspect_miss_w_to_b_start )) ? 
							((estimatated_region_end_b + estimatated_region_start_b) - (suspect_miss_w_to_b_count + 2 * suspect_miss_w_to_b_start ))
							: ((suspect_miss_w_to_b_count + 2 * suspect_miss_w_to_b_start ) - ( estimatated_region_end_b + estimatated_region_start_b ));
...
reg [10:0] suspect_miss_w_to_b_count, suspect_miss_b_to_w_count;
reg [10:0] suspect_miss_w_to_b_start, suspect_miss_b_to_w_start;
reg [6:0]  count_w_to_b_strip, count_b_to_w_strip;
...
always@(posedge clk) begin
		...
		//parameter MESSAGE_BUF_MAX = 256 parameter MSG_INTERVAL = 6;
			...
			if(whiteToBlack && whiteToBlack_1 && (prev_state == 0 || prev_state == 2))begin
				// | white - black | 
						...
						suc_white_start <= 0;
						prev_state <= 1;
						count_b_to_w_strip <= count_b_to_w_strip + 1;
				end
				...
				// | black - white |
						...
						suc_black_start <= 0;
						prev_state <= 0;
						count_w_to_b_strip <= count_w_to_b_strip + 1;
				end
				// else if(whiteToBlack && whiteToBlack_1 && (prev_state == 1) && suc_black_count > 30 )begin
				// // | white - black  ?  white-black | (black to white miss) Suspect, to increase trust
				// 		suc_black_start <= x;
				// 		suspect_miss_b_to_w_count <= suc_black_count;
				// 		suspect_miss_b_to_w_start <= suc_black_start;
				// 		suc_white_count <= 0;
				// 		suc_white_start <= 0;
				// 		prev_state <= 1;
				// 		count_b_to_w_strip <= count_b_to_w_strip + 1;
				...
				// else if(blackToWhite && blackToWhite_1 && (prev_state == 0) && suc_black_count > 30 )begin
				// // | black - white ?  black - white | (white to black miss) Suspect, to increase trust
				// 		suc_black_start <= x;
				// 		suspect_miss_w_to_b_count <= suc_white_count;
				// 		suspect_miss_b_to_w_start <= suc_white_start;
				// 		suc_black_count <= 0;
				// 		suc_black_start <= 0;
				// 		prev_state <= 0;
				// 		count_w_to_b_strip <= count_w_to_b_strip + 1;
				...
				if(pre_black_count >= pre_white_count)begin
					if(((pre_black_count - pre_white_count) < 10) && (pre_white_count > 15 && pre_black_count < 120 )) begin
                        ...
                        if(pre_black_start < pre_white_start && pre_white_start > pre_black_start + pre_black_count- 8 && pre_white_start < pre_black_start + pre_black_count + 8)begin
							if((max_gap + 6 < current_gap) && (current_gap < 130) && (pre_black_start > 50) && (pre_white_start + pre_white_count < 600) ) begin
							...
							if((max_gap + 6 < current_gap) && (current_gap < 130) && (pre_white_start > 50) && (pre_black_start + pre_black_count < 600))begin	
					...
					if(((pre_white_count - pre_black_count) < 10) && (pre_black_count > 15 && pre_white_count < 120) ) begin
						current_gap = pre_black_count + pre_white_count;
                        if(pre_black_start < pre_white_start && pre_white_start > pre_black_start + pre_black_count- 8 && pre_white_start < pre_black_start + pre_black_count + 8)begin
							if(max_gap + 6 < current_gap)begin
								max_start_edge_x_position_b <= pre_black_start;
								max_end_edge_x_position_b <= pre_white_start + pre_white_count;
								max_gap <= current_gap;
							end
	...
	if (x == IMAGE_W-1) begin
		...
		count_w_to_b_strip <= 0;
		count_b_to_w_strip <= 0;
		suspect_miss_b_to_w_count <= 0;
		suspect_miss_b_to_w_start <= 0;
		suspect_miss_w_to_b_count <= 0;
		suspect_miss_w_to_b_start <= 0;
