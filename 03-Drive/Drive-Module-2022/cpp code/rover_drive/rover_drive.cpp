...
void roverdrive::measure() {
    ...
    this->dx = this->dr*cos(this->phi);
    this->dy = this->dr*sin(this->phi);
    ...
    //Serial.println(this->squal);
    //Serial.println(this->r);
    //Serial.println(this->phi);
...
void roverdrive::translate(float rtarget) {
  ...
  if (sample) {
    ...
    if (std::abs(this->r) > std::abs(rtarget)) {
...
void roverdrive::rotate(float phitarget) {
  ...
  if (sample) {
    ...
    this->motorA(sgn*(-0.5-0.01*this->r));
    this->motorB(sgn*(0.5-0.01*this->r));

    if (std::abs(this->phi) > std::abs(phitarget)) {
